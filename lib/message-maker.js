...
var getMessageType = exports.getMessageType = function(req){
  ...
  if (req.body) {
    ...
    if (nick.length > 0) {
      ...
      return "activity";
    } else if (messageContent.match(privateMatch)) {
      var privateNick = messageContent.split(' ')[1].split(' ')[0];
      if (userList.indexOf(privateNick)) {
        return "private";
      }
    }
  }
...
exports.getMessage = function(noodle, req, io, userList, messageType) {
  ...
  // If this is a reply or a private message generate a new thread
  this.generateChannel = function() {
    channel.generateTemporary(messageContent);
  }
